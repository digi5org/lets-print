...
export default function RoleActivityPage() {
  ...
  const getRoleColor = (role) => {
    switch (role) {
      ...
      case "production_owner": return "bg-green-100 text-green-800";
  ...
  const filteredActivities = selectedRole === "all" 
    ? mockActivities 
    : mockActivities.filter(a => a.role === selectedRole);
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {["User Management", "Order Management", "Product Management", "Analytics", "Billing", "Settings"].map((perm) => (
                <tr key={perm}>
                  <td className="py-3 text-gray-900">{perm}</td>
                  <td className="py-3 text-center"><span className="text-green-500">✓</span></td>
                  <td className="py-3 text-center"><span className="text-green-500">✓</span></td>
                  <td className="py-3 text-center">{perm === "Order Management" || perm === "Product Management" ? <span className="text-green-500">✓</span> : <span className="text-red-500">✗</span>}</td>
                  <td className="py-3 text-center">{perm === "Order Management" ? <span className="text-green-500">✓</span> : <span className="text-red-500">✗</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
